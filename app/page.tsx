...
import { Menu, X, Zap, Check, ChevronDown, ChevronUp } from 'lucide-react';
...
const features = [
  { emoji: '📊', title: 'Métricas em tempo real', desc: 'Dashboards com indicadores da instituição atualizados automaticamente para cada perfil de acesso.' },
  { emoji: '👥', title: 'Gestão multi-perfil', desc: 'Perfis separados para diretor, coordenador, professor e aluno — cada um vê exatamente o que precisa.' },
  { emoji: '💰', title: 'Controle financeiro', desc: 'Mensalidades, inadimplência e fluxo de caixa em um painel completo. Nunca perca um pagamento.' },
  { emoji: '📝', title: 'Lançamento de notas', desc: 'Professores lançam notas por turma, disciplina e bimestre de forma simples, rápida e rastreável.' },
  { emoji: '📅', title: 'Controle de frequência', desc: 'Chamadas com status presente, ausente ou justificado. Histórico completo por aluno e turma.' },
  { emoji: '🔒', title: 'Segurança e confiabilidade', desc: 'Autenticação JWT, proteção de rotas por perfil, validação de dados e infraestrutura robusta.' },
];

const steps = [
  { emoji: '🏫', step: '01', title: 'Cadastre sua escola', desc: 'Crie sua conta em menos de 2 minutos. Preencha os dados da instituição e comece o trial.' },
  { emoji: '👥', step: '02', title: 'Convide sua equipe', desc: 'Adicione diretores, coordenadores, professores e alunos. Cada um recebe acesso personalizado.' },
  { emoji: '🚀', step: '03', title: 'Comece a usar', desc: 'Lance notas, registre chamadas, acompanhe o financeiro. Tudo funcionando desde o primeiro dia.' },
];
        ...
        <div className="rounded-xl p-4 border border-white/5 mb-4" style={{ background: '#0D0D14' }}>
          ...
          <div className="flex items-end gap-2 h-16">
            {[65, 80, 72, 88, 75, 90, 68, 85].map((h, i) => (
              <div key={i} className="flex-1 rounded-t-sm transition-all" style={{ height: `${h}%`, background: i % 2 === 0 ? 'rgba(59,130,246,0.6)' : 'rgba(139,92,246,0.6)' }} />
            ))}
          </div>
        </div>
        {/* List */}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-6 py-5">
        ...
        {open ? <ChevronUp size={16} style={{ color: '#3B82F6' }} /> : <ChevronDown size={16} style={{ color: '#64748B' }} />}
      </div>
      {open && (
    ...
    <div className={inter.className} style={{ background: '#0A0A0F', minHeight: '100vh', color: '#F1F5F9' }}>
      ...
      <header style={{ background: 'rgba(10,10,15,0.85)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'sticky', top: 0, zIndex: 50 }}>
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <Link href="/cadastro" className="text-sm px-4 py-2 rounded-xl font-semibold text-white glow-btn"
      ...
      <section className="pt-20 pb-24 px-6">
        <div ref={heroRef} style={fadeStyle} className="max-w-5xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-6">
            <Link href="/cadastro" className="px-8 py-4 rounded-2xl font-bold text-white text-base glow-btn"
              style={{ background: 'linear-gradient(135deg,#3B82F6,#8B5CF6)' }}>
              Começar 14 dias grátis
            </Link>
            <Link href="/planos" className="px-8 py-4 rounded-2xl font-medium text-sm border transition-colors"
            ...
            {steps.map((s) => (
              <div key={s.step} className="text-center">
                <div className="relative inline-flex mb-5">
                  <div className="w-20 h-20 rounded-2xl flex items-center justify-center text-3xl border"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-stretch">
            ...
            <div className="gradient-border flex flex-col" style={{ position: 'relative' }}>
              ...
              <div className="rounded-[18px] p-8 flex flex-col h-full" style={{ background: '#111118' }}>
                <span className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ background: 'linear-gradient(90deg,#60A5FA,#A78BFA)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Pro</span>
                <div className="flex items-end gap-1 mb-1">
                ...
                <Link href="/planos" className="w-full text-center py-3 rounded-xl font-bold text-sm text-white glow-btn"
                  style={{ background: 'linear-gradient(135deg,#3B82F6,#8B5CF6)' }}>
                  Assinar agora
                </Link>
              </div>
      ...
      <section id="contato" className="py-24 px-6" style={{ background: '#0D0D14' }}>
        <div ref={ctaRef} style={fadeStyle} className="max-w-2xl mx-auto text-center">
          <div className="rounded-3xl border p-12" style={{ background: 'linear-gradient(135deg, rgba(59,130,246,0.08), rgba(139,92,246,0.08))', borderColor: 'rgba(59,130,246,0.2)' }}>
            ...
            <Link href="/cadastro" className="inline-block px-10 py-4 rounded-2xl font-bold text-white text-base glow-btn"
              style={{ background: 'linear-gradient(135deg,#3B82F6,#8B5CF6)' }}>
