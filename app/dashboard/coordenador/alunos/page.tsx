...
export default function CoordenadorAlunosPage() {
  ...
  const turmas = Array.from(new Set(students.map(s => s.className).filter(Boolean))) as string[];

  const filtered = students.filter(s => {
    const matchSearch = s.name.toLowerCase().includes(search.toLowerCase()) ||
      s.email.toLowerCase().includes(search.toLowerCase());
    const matchTurma = !turmaFilter || s.className === turmaFilter;
                      ...
                      <td className="px-4 sm:px-6 py-3.5">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400 dark:text-gray-500 sm:hidden">{s.className ?? '—'}</p>
                          </div>
                        </div>
                      </td>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="right" className="flex flex-col p-0">
          <SheetHeader>
            ...
            <SheetDescription>{detail?.className ? `Turma ${detail.className}` : 'Sem turma'}</SheetDescription>
          </SheetHeader>
              ...
              <div className="space-y-4">
                ...
                {detail.className && (
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
                    ...
                    <p className="text-sm font-medium text-gray-700 dark:text-gray-200">{detail.className}</p>
                    {detail.classYear && (
                      <p className="text-xs text-gray-400 mt-0.5">Ano letivo: {detail.classYear}</p>
                    )}
                  </div>
                )}
              </div>
