      ...
      <header className="bg-white border-b border-gray-100 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-bold text-[#1E3A5F]">Alunos</h1>
          </div>
          <button
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400 text-sm">Nenhum aluno encontrado</div>
          ) : (
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase hidden md:table-cell">Turma</th>
                  <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Status Financeiro</th>
                </tr>
              </thead>
                ...
                {filtered.map((s) => (
                  <tr key={s.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 hidden md:table-cell">
                      <span className="text-sm text-gray-500">{s.class?.name ?? '—'}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${financialStatusColor[s.financialStatus] ?? 'bg-gray-50 text-gray-600'}`}>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold text-[#1E3A5F] mb-4">Matricular aluno</h2>
