    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <header className="bg-white border-b border-gray-100 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-gray-500">Painel da Secretaria</span>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Secretária — aqui está o resumo da escola hoje.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-blue-500 font-medium bg-blue-50 px-2 py-1 rounded-full">Ativas</span>
            </div>
            <p className="text-3xl font-bold text-[#1E3A5F]">{data?.totalClasses ?? 0}</p>
            <p className="text-sm text-gray-500 mt-1">Total de Turmas</p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              ...
              {(data?.totalDefaulters ?? 0) > 0 && (
                <span className="text-xs text-red-500 font-medium bg-red-50 px-2 py-1 rounded-full">Atenção</span>
              )}
            </div>
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-orange-500 font-medium bg-orange-50 px-2 py-1 rounded-full">Este mês</span>
            </div>
            <p className="text-3xl font-bold text-[#1E3A5F]">{data?.enrollmentsThisMonth ?? 0}</p>
            <p className="text-sm text-gray-500 mt-1">Matrículas do Mês</p>
          </div>
        </div>
        ...
        {data && data.totalDefaulters > 0 && (
          <div className="bg-red-50 border border-red-100 rounded-2xl p-4 flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-red-500">Verifique o financeiro para mais detalhes</p>
            </div>
            <button
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: 'Alunos', icon: Users, href: '/dashboard/secretaria/alunos', color: 'bg-blue-50 text-blue-600' },
            { label: 'Turmas', icon: BookOpen, href: '/dashboard/secretaria/turmas', color: 'bg-purple-50 text-purple-600' },
            { label: 'Financeiro', icon: DollarSign, href: '/dashboard/secretaria/financeiro', color: 'bg-emerald-50 text-emerald-600' },
            { label: 'Usuários', icon: UserPlus, href: '/dashboard/secretaria/usuarios', color: 'bg-orange-50 text-[#F97316]' },
          ].map((item) => (
            <button
              key={item.label}
