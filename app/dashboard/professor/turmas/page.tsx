      ...
      <header className="bg-white border-b border-gray-100 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-bold text-[#1E3A5F]">Minhas Turmas</h1>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {classes.map((c) => (
              <div key={c.id} className="bg-white rounded-2xl border border-gray-100 p-6 hover:shadow-sm transition-shadow">
                ...
                <p className="text-xs text-gray-400 mb-1">{c.year}</p>

                {c.totalStudents !== undefined && (
                  <div className="flex items-center gap-1 mb-4">
                    <Users size={13} className="text-gray-400" />
                    <span className="text-xs text-gray-500">{c.totalStudents} alunos</span>
                  </div>
                )}
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {c.subjects.slice(0, 3).map((s) => (
                        <span key={s.id} className="text-xs px-2 py-0.5 bg-blue-50 text-blue-600 rounded-full">
                          {s.name}
                        </span>
                      ))}
                      {c.subjects.length > 3 && (
                ...
                <div className="grid grid-cols-2 gap-2 pt-3 border-t border-gray-50">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 py-2 rounded-xl bg-green-50 text-green-600 text-xs font-medium hover:bg-green-100 transition-colors"
                  >
                    <CheckSquare size={14} />
                    Chamada
                  </button>
