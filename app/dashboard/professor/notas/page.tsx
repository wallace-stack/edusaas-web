...
export default function LancarNotasPage() {
  ...
  const [form, setForm] = useState({
    classId: '', subjectId: '', studentId: '', value: '',
    type: 'exam', period: '1', description: '',
  });
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-gray-950">
      <header className="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-6 py-4">
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <h1 className="font-bold text-[#1E3A5F] dark:text-white">Lançar Notas</h1>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        ...
        {success && (
          <div className="bg-green-50 dark:bg-green-950 border border-green-100 dark:border-green-800 rounded-2xl p-4 flex items-center gap-3 mb-6">
            ...
            <p className="text-sm text-green-700 dark:text-green-300 font-medium">Nota lançada com sucesso!</p>
          </div>
        )}

        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6">
          <h2 className="font-semibold text-[#1E3A5F] dark:text-white mb-6">Dados da avaliação</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Aluno</label>
              <select
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Nota (0-10)</label>
                <input
                  value={form.value}
                  onChange={(e) => setForm({ ...form, value: e.target.value })}
                  type="number" min="0" max="10" step="0.1" required placeholder="Ex: 8.5"
                  className={inputCls}
                />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Bimestre</label>
