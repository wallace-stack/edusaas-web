    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <header className="bg-white border-b border-gray-100 px-6 py-4">
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <h1 className="font-bold text-[#1E3A5F]">Registrar Chamada</h1>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 py-8">
        ...
        {success && (
          <div className="bg-green-50 border border-green-100 rounded-2xl p-4 flex items-center gap-3 mb-6">
            ...
            <p className="text-sm text-green-700 font-medium">Chamada registrada com sucesso!</p>
          </div>
        )}

        {/* Configuração da chamada */}
        <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
          <h2 className="font-semibold text-[#1E3A5F] mb-4">Configurar chamada</h2>
          ...
          <form onSubmit={handleSubmit}>
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden mb-4">
              ...
              <div className="px-6 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                <span className="text-sm font-medium text-gray-600">{students.length} alunos</span>
                    ...
                    <div key={student.id} className="px-6 py-4 flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`p-2 rounded-xl transition-colors ${attendance?.status === 'present' ? 'bg-green-100 text-green-600' : 'text-gray-300 hover:text-green-500'}`}
                          title="Presente"
                        >
                          <CheckCircle size={22} />
                        </button>
                        <button
                          ...
                          className={`p-2 rounded-xl transition-colors ${attendance?.status === 'absent' ? 'bg-red-100 text-red-600' : 'text-gray-300 hover:text-red-500'}`}
                          title="Ausente"
                        >
                          <XCircle size={22} />
                        </button>
                        <button
                          ...
                          className={`p-2 rounded-xl transition-colors ${attendance?.status === 'justified' ? 'bg-orange-100 text-orange-600' : 'text-gray-300 hover:text-orange-500'}`}
                          title="Justificado"
                        >
                          <MinusCircle size={22} />
                        </button>
        ...
        {form.classId && students.length === 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
            <p className="text-gray-400 text-sm">Nenhum aluno encontrado nesta turma</p>
          </div>
        )}
